...
export default function App() {
  const[products, setProducts] = useState(() => JSON.parse(localStorage.getItem('dk_products')) || []);
  const[cart, setCart] = useState(() => JSON.parse(localStorage.getItem('dk_cart')) || []);
  const[myOrderIds, setMyOrderIds] = useState(() => JSON.parse(localStorage.getItem('dk_my_order_ids')) || []);
  const[orders, setOrders] = useState(() => JSON.parse(localStorage.getItem('dk_orders')) ||[]);
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const localProducts = JSON.parse(localStorage.getItem('dk_products')) ||[];

    // Check if cache is still valid
    if (lastFetchTime && (now - parseInt(lastFetchTime)) < CACHE_EXPIRY_MS && localProducts.length > 0) {
      console.log("✅ Using cached Products & Settings (Saves API calls)");
      return; 
    }
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      if (myOrderIds.length === 0 || GOOGLE_SCRIPT_URL === "YOUR_GOOGLE_SCRIPT_WEB_APP_URL_HERE") return;

      const ORDER_CACHE_MS = 3 * 60 * 1000; // 3 Minutes
      const lastOrderFetch = localStorage.getItem('dk_last_order_fetch');
      const now = Date.now();

      // Check order cache
      if (lastOrderFetch && (now - parseInt(lastOrderFetch)) < ORDER_CACHE_MS) {
        console.log("✅ Using cached Orders data");
        return;
      }

      try {
        const res = await fetch(`${GOOGLE_SCRIPT_URL}?action=getMyOrders&ids=${myOrderIds.join(',')}`);
        ...
        if(data.orders) {
          ...
          // Save new order cache timestamp
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl md:rounded-3xl p-5 md:p-8 flex flex-col-reverse md:flex-row items-center justify-between gap-5 shadow-xl relative overflow-hidden">
        ...
        <div className="w-full h-32 sm:h-40 md:w-2/5 md:h-56 relative z-10 shrink-0">
          <img 
            src={settings.heroImage} 
            alt="Banner" 
            className="w-full h-full object-cover rounded-xl md:rounded-2xl shadow-xl transform md:rotate-2 hover:rotate-0 transition duration-500 border border-white/10 bg-gray-800" 
          />
        </div>
      </div>
