...
export default function App() {
      ...
      .then(data => {
        ...
        if (data.products) {
          const formattedProducts = data.products.map(p => {
            // FIX: Auto-correct Category Names (handles uppercase/lowercase and extra spaces)
            let rawCat = (p.category || '').toString().trim();
            let exactCat = CATEGORIES.find(c => c.toLowerCase() === rawCat.toLowerCase()) || rawCat;
  ...
  useEffect(() => {
    ...
    fetchOrders();
  }, [myOrderIds]); // Safe dependency array
    ...
    <StoreContext.Provider value={contextValue}>
      <div className="min-h-screen bg-gray-50 text-gray-800 font-sans pb-16 md:pb-0 relative">
        ...
        <footer className="bg-white border-t border-gray-200 mt-8 md:mt-12 pb-20 md:pb-6">
          <div className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm text-gray-600">
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      ...
      {homeCategories.map(cat => {
        ...
        const catProducts = safeProducts.filter(p => (p.category||'').toString().trim().toLowerCase() === cat.toLowerCase()).slice(0, 5); 
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        {CATEGORIES.map(cat => {
          const count = safeProducts.filter(p => (p.category||'').toString().trim().toLowerCase() === cat.toLowerCase()).length;
...
function ProductListView() {
  ...
  const filtered = safeProducts.filter(p => {
    ...
    const matchCategory = activeCategory === 'All' || pCat === activeCategory.toLowerCase();
    const matchSearch = (p.title || '').toLowerCase().includes(searchQuery.toLowerCase()) || pCat.includes(searchQuery.toLowerCase());
